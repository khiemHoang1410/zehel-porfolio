...
export default function TechList({ techs, onEdit }: { techs: ITech[], onEdit: (tech: ITech) => void }) {
    ...
    if (!techs || techs.length === 0) return (
        <div className="text-center py-12 border-4 border-dashed border-gray-200 rounded-lg">
            <p className="text-gray-400 font-black uppercase text-xl">EMPTY ARMORY</p>
            <p className="text-sm text-gray-400">Chưa có công nghệ nào được nạp đạn.</p>
        </div>
    );
        ...
        <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-5 pb-10">
            {techs.map((tech) => (
                ...
                >
                    ...
                    <span className={`absolute -top-3 -left-2 text-[9px] font-black uppercase border-2 border-black px-2 py-0.5 shadow-sm ${categoryColors[tech.category] || 'bg-white'}`}>
                        {tech.category}
                    </span>

                    {/* 2. Ngôi sao Featured (Góc phải) */}
                    {tech.isFeatured && (
                        <div className="absolute -top-3 -right-2 bg-black text-yellow-400 p-1 rounded-full border-2 border-yellow-400 shadow-sm z-10" title="Featured Skill">
                    ...
                    <div className="text-center w-full">
                        <h4 className="font-black text-sm uppercase truncate w-full mb-1" title={tech.name}>
                            {tech.name}
                        </h4>
